split_trip <- function(gtfs, trip, method = 'equal.size', split = 1){
  ...
  gtfs$stop_times <- 
    split_data %>% 
    dplyr::select(-dupe) %>% 
    dplyr::mutate(trip_id = paste0(trip_id, '.', LETTERS[subtrip])) %>% 
    dplyr::group_by(trip_id) %>% 
  ...
  routes_dic <-  # verificar se precisa disso tudo
    gtfs$trips %>% 
    dplyr::select(trip_id, new.trip_id, route_id) %>% 
    na.omit()
  ...
  if (!is_null(gtfs$frequencies)) {
    ...
    gtfs$frequencies <- 
      dplyr::left_join(gtfs$frequencies, trip.dic) %>% 
      dplyr::mutate(trip_id = if_else(is.na(new.trip_id), trip_id, new.trip_id))%>% 
      dplyr::select(-new.trip_id)
  ...
  if (!is_null(gtfs$transfers)) {
    ...
    gtfs$transfers <- 
      dplyr::left_join(gtfs$transfers, trip.dic) %>% 
      dplyr::mutate(trip_id = if_else(is.na(new.trip_id), trip_id, new.trip_id)) %>% 
      dplyr::select(-new.trip_id)
