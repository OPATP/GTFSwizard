as_shapes_sf <- function(obj){
  UseMethod('as_shapes_sf')
}

as_shapes_sf.wizardgtfs <- function(obj){
  if('shapes' %in% names(obj) == FALSE){
    warning("Gtfs doesn't have a shapes table, using ", crayon::blue("get_shapes"), " to build it")
    obj <- get_shapes(obj)
  }
  obj$shapes <- as_shapes_sf.data.frame(obj$shapes)
  return(obj)
}

as_shapes_sf.list <- function(obj){
  if('shapes' %in% names(obj) == FALSE){
    warning("Gtfs doesn't have a shapes table, using ", crayon::blue("get_shapes"), " to build it")
    obj <- get_shapes(obj)
  }
...
as_shapes_sf.gtfs <- function(obj){
  if('shapes' %in% names(obj) == FALSE){
    warning("Gtfs doesn't have a shapes table, using ", crayon::blue("get_shapes"), " to build it")
    obj <- get_shapes(obj)
  }
...
as_shapes_sf.data.frame <- function(obj){
  if('sf'%in%class(obj)){
    st_crs(obj) <- 4326
    return(obj)
  }else{
